    ...
    <section className=" relative mt-2 px-10 py-28 max-md:px-5  h-full min-h-screen max-lg:pt-5 max-lg:mt-20">
     ...
     {!isActive_3 && <div
      ...
      >
        ...
        <Link
          to="/"
          className="border rounded-xl text-primary shadow-lg p-2 mt-16 vazir-regular max-md:mt-10"
        >
          بازگشت به صفحه خانه
        </Link>
      </div>}
      ...
      >
        ...
        <div className="flex justify-center items-center pl-28 max-md:pl-5">
          {isActive_3 ? (
            <img
              src={cardTickOrangeIcon}
              className="w-5 h-5 ml-2"
              alt="سبد خرید"
            />
          ) : (
            <img
              src={CardTickGrayIcon}
              className="w-5 h-5 ml-2"
              alt="سبد خرید"
            />
          )}
